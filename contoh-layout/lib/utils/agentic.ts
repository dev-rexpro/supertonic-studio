...
function deriveSingleTurnSections(
	...
	// Fallback: If reasoningContent is empty but content contains think/thought/thinking tags,
	// extract them dynamically for correct UI display.
	...
	// 1. Reasoning content (from dedicated field or fallback)
	if (reasoning) {
		const toolCalls = parseToolCalls(message.toolCalls);
		const hasContentAfterReasoning =
			!!content.trim() || toolCalls.length > 0 || streamingToolCalls.length > 0;
