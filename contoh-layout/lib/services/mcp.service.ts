...
export class MCPService {
			...
			fetch: async (input, init) => {
				if (useProxy && typeof window !== 'undefined') {
					...
					if (requestUrlStr) {
						...
						if (!parsedRequestUrl.pathname.includes(CORS_PROXY_ENDPOINT)) {
							let targetUrlStr = requestUrlStr;
							if (parsedRequestUrl.origin === window.location.origin) {
								...
								targetUrlStr = realTargetUrl.href;
							}
							const proxiedUrl = buildProxiedUrl(targetUrlStr);
	...
	static createTransport(
		...
		const isSSE = config.transport === MCPTransportType.SSE;
		...
		// Otherwise, explicitly use Streamable HTTP
		if (import.meta.env.DEV && import.meta.env.VITE_DEBUG) {
	...
	static async connect(
		...
		const isRemoteSse =
			...
			(serverConfig.url.startsWith('http://') &&
				!serverConfig.url.includes('localhost') &&
				!serverConfig.url.includes('127.0.0.1') &&
				!serverConfig.url.includes('::1'));

		if (isRemoteSse) {
			// Automatically use proxy for remote HTTP MCP connections to prevent CORS blocking universally.
		...
		if (transportType === MCPTransportType.WEBSOCKET) {
			(transport as WebSocketClientTransport).onclose = () => {
		...
		try {
			...
			// If transport type was not explicitly configured and we attempted Streamable HTTP, fall back to SSE
			if (!serverConfig.transport && transportType === MCPTransportType.STREAMABLE_HTTP) {
				console.warn(
					`[MCPService][${serverName}] StreamableHTTP connection failed, falling back to SSE...`,
					error
				);
				try {
					const fallbackConfig = { ...serverConfig, transport: MCPTransportType.SSE };
					const fallback = this.createTransport(serverName, fallbackConfig, (log) =>
						onPhase?.(log.phase, log)
					);
					transport = fallback.transport;
					transportType = fallback.type;
					stopPhaseLogging = fallback.stopPhaseLogging;

					onPhase?.(
						MCPConnectionPhase.TRANSPORT_READY,
						this.createLog(
							MCPConnectionPhase.TRANSPORT_READY,
							`Fallback Transport ready (${transportType})`
						),
						{ transportType }
					);

					// Create a new client to connect since the old client may have stored partial state
					client = new Client(
						{
							name: effectiveClientInfo.name,
							version: effectiveClientInfo.version ?? DEFAULT_CLIENT_VERSION
					...
					onPhase?.(
						...
						this.createLog(
							MCPConnectionPhase.INITIALIZING,
							'Sending initialize request (SSE fallback)...'
						)
					);
					...
					client.onerror = runtimeErrorHandler;
					// Throw the original StreamableHTTP error, it is usually more descriptive
					throw error;
				}
			} else {
				client.onerror = runtimeErrorHandler;
