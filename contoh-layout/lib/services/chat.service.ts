...
import { resolveProviderApiUrl, getProviderJsonHeaders } from '$lib/utils/cors-proxy';
...
function isProviderMode(): boolean {
	...
	return current !== null && current.providerName !== 'llamacpp';
}
...
const START_TAGS = ['<think>', '<thought>', '<thinking>'];
const END_TAGS = ['</think>', '</thought>', '</thinking>'];
...
function processReasoningFromContent(
	...
	if (inThinkingBlock) {
		...
		if (firstEndIdx !== -1) {
			const reasoning = thinkingBuffer + text.substring(0, firstEndIdx);
			const remaining = text.substring(firstEndIdx + matchedEndTag.length);
		...
		return {
			...
			newThinkingBuffer: thinkingBuffer + text
		};
	}
	...
	const before = text.substring(0, firstStartIdx);
	const after = text.substring(firstStartIdx + matchedStartTag.length);
	...
	const thinkContent = after.substring(0, firstEndIdx);
	const afterEnd = after.substring(firstEndIdx + matchedEndTag.length);
...
export class ChatService {
	...
	static async sendMessage(
		...
		const resolvedModel = options.model || modelsStore.selectedModelName;
		if (resolvedModel && !modelsStore.modelSupportsVision(resolvedModel)) {
			normalizedMessages.forEach((msg) => {
				if (Array.isArray(msg.content)) {
					msg.content = msg.content.filter((part: ApiChatMessageContentPart) => {
						if (part.type === ContentPartType.IMAGE_URL) {
							console.info(
								`[ChatService] Skipping image attachment in message history (model "${resolvedModel}" does not support vision)`
							);
		...
		const requestBody: ApiChatCompletionRequest = {
			messages: normalizedMessages.map((msg: ApiChatMessageData) => {
				...
				if (!isProviderMode() && !excludeReasoningFromContext && msg.reasoning_content) {
		...
		if (selectedPersona) {
			(requestBody as any).selectedPersona = selectedPersona;
		}
		...
		if (max_tokens !== undefined) {
			if (isProviderMode()) {
				// For remote API providers, omit max_tokens if 0 or null to let them use their default max tokens limit.
				// Some API providers fail if max_tokens is negative or 0.
		...
		// Add tools_config to payload for server-side completions API
		requestBody.tools_config = {
			toolsMode: toolsConfigStore.toolsMode,
			structuredOutputsEnabled: toolsConfigStore.structuredOutputsEnabled,
			structuredOutputsSchema: toolsConfigStore.structuredOutputsSchema,
			codeExecutionEnabled: toolsConfigStore.codeExecutionEnabled,
			functionCallingEnabled: toolsConfigStore.functionCallingEnabled,
		...
		try {
			const headers: Record<string, string> = { ...getProviderJsonHeaders() };
			...
			const response = await fetch(resolveProviderApiUrl(API_CHAT.COMPLETIONS), {
				method: 'POST',
				headers,
	...
	static async stopReasoning(completionId: string, model?: string | null): Promise<boolean> {
		...
		try {
			const res = await fetch(resolveProviderApiUrl(API_CHAT.CONTROL), {
				method: 'POST',
				headers: getProviderJsonHeaders(),
	...
	static async preEncode(
		...
		const requestBody: Record<string, unknown> = {
			messages: normalizedMessages.map((msg: ApiChatMessageData) => {
				...
				if (!isProviderMode() && !excludeReasoning && msg.reasoning_content) {
					mapped.reasoning_content = msg.reasoning_content;
				}
		...
		try {
			await fetch(resolveProviderApiUrl(API_CHAT.COMPLETIONS), {
				method: 'POST',
				headers: getProviderJsonHeaders(),
				body: JSON.stringify(requestBody),
				signal
							...
							try {
								...
								if (timings) {
									if (reasoningStartTime !== undefined) {
										...
										timings.reasoning_ms = Math.max(0, endTime - reasoningStartTime);
									}
