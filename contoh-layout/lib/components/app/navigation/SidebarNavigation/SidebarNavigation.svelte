<script lang="ts">
	...
	$effect(() => {
		if (isMobile.current && page.url.pathname.includes(ROUTES.SEARCH)) {
...
{#if innerWidth > 768 || (!page.url.pathname.startsWith(ROUTES.SETTINGS) && page.url.pathname !== ROUTES.MCP_SERVERS && page.url.pathname !== ROUTES.SEARCH)}
	<aside
		class={[
			// Layout & positioning
			...
			>
				...
				{:else}
					<a
						...
						class={[
							"pointer-events-auto flex items-center rounded-lg bg-transparent hover:bg-foreground/5 dark:hover:bg-foreground/10 transition-colors",
							isExpandedMode ? 'pl-1 pr-2.5 gap-2 h-9 w-auto justify-start' : 'md:h-9 md:w-9 h-10 w-10 justify-center'
						]}
					...
					>
						...
						{#if isExpandedMode}
							<span class="font-semibold tracking-tight text-foreground select-none" style="font-size: 18px;" in:fade={{ duration: 150 }}>
								Masbro
							</span>
						{/if}
					</a>
			...
			{#if isOnMobile || (isExpandedMode && !alwaysShowOnDesktop)}
				...
				>
					<ActionIcon
						...
						class="backdrop-blur-none md:h-9 md:w-9 h-10 w-10 rounded-lg mr-1 hover:bg-accent!"
						onclick={toggleExpandedMode}
