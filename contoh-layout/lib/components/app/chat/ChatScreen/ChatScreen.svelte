<script lang="ts">
	...
	async function handleSendMessage(
		message: string,
		files: ChatUploadedFile[] | undefined
	): Promise<boolean> {
	...
	>
		...
		>
			...
			<div class="pointer-events-none flex flex-col gap-6 items-center w-full">
				{#if (isMobile.current ? mobileScrollDownHint || isMobileUserScrolledUp : autoScroll.userScrolledUp) && page.url.pathname.startsWith(ROUTES.CHAT) && page.params.id}
