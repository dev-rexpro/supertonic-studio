<script lang="ts">
	...
	function getTurnStatsForSectionIndex(index: number) {
		for (let turnIdx = 0; turnIdx < turnGroups.length; turnIdx++) {
			if (turnGroups[turnIdx].flatIndices.includes(index)) {
				return message?.timings?.agentic?.perTurn?.[turnIdx];
			}
		}
		return undefined;
	}

	function getReasoningTitle(index: number, section: AgenticSection): string {
		...
		const turnStats = turnGroups.length > 1 ? getTurnStatsForSectionIndex(index) : undefined;
		const reasoningMs = turnStats?.llm?.reasoning_ms ?? message?.timings?.reasoning_ms;
		...
		const seconds = Math.round(reasoningMs / 1000);
		if (seconds === 1) {
...
{#snippet renderSection(section: (typeof sectionsParsed)[number], index: number)}
	...
	{:else if section.type === AgenticSectionType.REASONING}
		<CollapsibleContentBlock
			open={isExpanded(index, section)}
			class="mt-0.5 mb-0.5"
			title={getReasoningTitle(index, section)}
			onToggle={() => toggleExpanded(index, section)}
			{isStreaming}
			classContent={config().renderThinkingAsMarkdown
				? 'break-words leading-8'
				: 'whitespace-pre-wrap break-words leading-8'}
		>
			{#if config().renderThinkingAsMarkdown}
