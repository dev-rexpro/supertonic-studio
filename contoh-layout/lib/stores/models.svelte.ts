...
export const GEMINI_MODELS: ModelOption[] = [
	{
		id: 'masbro-3-thinker',
		name: 'Masbro 3 Thinker',
		model: 'masbro-3-thinker',
		description: 'Thinking model for complex reasoning tasks (gemini-3.1-pro-preview / fallback gemini-2.5-pro)',
		capabilities: ['text', 'vision', 'audio', 'video'],
		modalities: { vision: true, audio: true, video: true },
		parsedId: {
			raw: 'masbro-3-thinker',
			orgName: 'google',
			modelName: 'masbro-3-thinker',
			params: null,
			activatedParams: null,
			quantization: null,
			tags: ['thinking', 'pro']
		},
		aliases: ['Masbro 3 Thinker'],
		tags: ['google', 'multimodal', 'thinking', 'pro'],
		capabilitiesSupported: ['Thinking', 'Search grounding', 'Grounding with Google Maps', 'Code execution', 'Structured outputs', 'Function calling', 'URL context'],
		capabilitiesNotSupported: [],
		tokenLimits: { input: '2M', output: '8k' }
	},
	{
		id: 'masbro-3',
		name: 'Masbro 3',
		model: 'masbro-3',
		description: 'Fast, balanced, and general-purpose model (gemini-3.5-flash / fallback gemini-2.5-flash)',
		capabilities: ['text', 'vision', 'audio', 'video'],
		modalities: { vision: true, audio: true, video: true },
		parsedId: {
			raw: 'masbro-3',
		...
		aliases: ['Masbro 3'],
		tags: ['google', 'multimodal', 'general'],
		capabilitiesSupported: ['Search grounding', 'Grounding with Google Maps', 'Code execution', 'Structured outputs', 'Function calling', 'URL context'],
		capabilitiesNotSupported: [],
		tokenLimits: { input: '1M', output: '8k' }
	},
	{
		id: 'masbro-3-fast',
		name: 'Masbro 3 Fast',
		model: 'masbro-3-fast',
		description: 'Ultra-fast, light model for quick responses (gemini-3.1-flash-lite / fallback gemini-2.5-flash-lite)',
		capabilities: ['text', 'vision', 'audio', 'video'],
		modalities: { vision: true, audio: true, video: true },
		parsedId: {
			raw: 'masbro-3-fast',
			orgName: 'google',
			modelName: 'masbro-3-fast',
			params: null,
			activatedParams: null,
			quantization: null,
			tags: ['lite', 'fast']
		},
		aliases: ['Masbro 3 Fast'],
		tags: ['google', 'multimodal', 'lite', 'fast'],
		capabilitiesSupported: ['Search grounding', 'Grounding with Google Maps', 'Code execution', 'Structured outputs', 'Function calling', 'URL context'],
		capabilitiesNotSupported: [],
		tokenLimits: { input: '1M', output: '8k' }
	}
...
class ModelsStore {
	...
	getModelModalities(modelId: string): ModelModalities | null {
		...
		// Fallback heuristic for remote/provider models
		const id = model?.model || model?.id || modelId;
		if (id) {
			...
			const hasVision =
				...
				lowered.includes('scout') ||
				lowered.includes('multimodal') ||
				(lowered.includes('qwen') && lowered.includes('vl'));

			const hasAudio = lowered.includes('audio') || lowered.includes('gemini-');
			const hasVideo = lowered.includes('video') || lowered.includes('gemini-');

			if (hasVision || hasAudio || hasVideo) {
				return { vision: hasVision, audio: hasAudio, video: hasVideo };
			}
		}
	...
	private async runFetch(): Promise<void> {
		...
		try {
			...
			if (!this.selectedModelId || !this.models.some((m) => m.id === this.selectedModelId)) {
				this.selectedModelId = 'masbro-3';
				this.selectedModelName = 'masbro-3';
			}
		} catch (error) {
			this.models = [];
	...
	private async fetchProviderModels(
		_providerConfig: ReturnType<typeof getProviderConfig>
	): Promise<void> {
		this.models = GEMINI_MODELS;
		this.routerModels = [];
	}
	...
	async fetchModelProps(modelId: string): Promise<ApiLlamaCppServerProps | null> {
		...
		const providerConfig = getProviderConfig();
		if (providerConfig && providerConfig.providerName !== 'llamacpp') return null;
	...
	async ensureFirstModelSelected(): Promise<void> {
		if (this.selectedModelName && this.models.some((m) => m.model === this.selectedModelName)) return;
	...
	async loadModel(modelId: string): Promise<void> {
		const providerConfig = getProviderConfig();
		if (providerConfig && providerConfig.providerName !== 'llamacpp') return;
