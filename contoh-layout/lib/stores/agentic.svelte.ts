		...
		while (true) {
			...
			try {
				...
				if (turnTimings) {
					...
					if (turnTimings.reasoning_ms) {
						agenticTimings.llm.reasoning_ms =
							(agenticTimings.llm.reasoning_ms || 0) + turnTimings.reasoning_ms;
					}
					...
					turnStats.llm.reasoning_ms = turnTimings.reasoning_ms;
				}
			} catch (error) {
				if (signal?.aborted) {
