...
class ChatStore {
	...
	private async streamChatCompletion(
		...
		if (isRouterMode()) {
			...
			const isValidModel =
				conversationModel && modelsStore.models.some((m) => m.model === conversationModel);
			effectiveModel =
				modelOverride ||
				selectedModelName() ||
				(isValidModel ? conversationModel : undefined) ||
		...
		const resolvedModelForVision = effectiveModel || modelsStore.selectedModelName;
		if (resolvedModelForVision && !modelsStore.modelSupportsVision(resolvedModelForVision)) {
			const hasImages = allMessages.some((msg) =>
				msg.extra?.some((extra) => extra.type === AttachmentType.IMAGE)
			);

			if (hasImages) {
				toast.error(
					`Model ${resolvedModelForVision} does not support image input. Images will be excluded.`
				);
			}
		}
	...
	async continueAssistantMessage(messageId: string): Promise<void> {
		...
		try {
			...
			const resolvedModelForVision = modelsStore.selectedModelName;
			if (resolvedModelForVision && !modelsStore.modelSupportsVision(resolvedModelForVision)) {
				const hasImages = contextWithContinue.some((msg) =>
					msg.extra?.some((extra: DatabaseMessageExtra) => extra.type === AttachmentType.IMAGE)
				);
	...
	private getApiOptions(): Record<string, unknown> {
		...
		apiOptions.selectedPersona = currentConfig.selectedPersona || 'standar';
